...
class Market1501(dataset.Dataset):
    def __init__(self, transform, dtype, data_path, crop_size=[128, 128],scale_factor=0.25,
        ...
        if dtype == 'train':
            self.data_path +=  '/train'#'/cloth-change_train'#'/cloth-unchange_train'#'/cloth-change_train'  # 'new_train'
        elif dtype == 'test':
            self.data_path +=  '/test' # 'new_A' #'/cloth-change_test' 
        else:
            self.data_path +=   '/query'#'/cloth-change_query' #'new_C' # 'new_B' 
        
        
        self.parsing_path = data_path + '/train_parsing' # '/cloth-change_train_parsing' #'new_train_parsing' # 
    ...
    def __getitem__(self, index):
        ...
        if self.type == 'train':
            img_erasing = DistributedErasing(probability=0.4)
    ...
    def camera(file_path):
        ...
        :return: camera id
        """
        return int((file_path.split('/')[-1].split('_')[2])[1])-1
