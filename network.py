...
from utils.matching_vector import MatchingVector
...
class MGN(nn.Module):
    def __init__(self):
        ...
        self.Matching_Vector = MatchingVector(num_classes)
    ...
    def forward(self, x):
        ...
        part_pd_score, matching_vector = self.Matching_Vector(x)
        # a = torch.ones_like(matching_vector) + matching_vector
        # b = torch.ones_like(matching_vector) - matching_vector
        # weights = torch.where(matching_vector>0.5, a, b)
        n_g1 = torch.mm(matching_vector,fg_p1)
        n_g2 = torch.mm(matching_vector,fg_p2)
        n_g3 = torch.mm(matching_vector,fg_p3)
        wieighted_predict = torch.cat([n_g1, n_g2, n_g3, f0_p2, f1_p2, f0_p3, f1_p3, f2_p3], dim=1)

        return predict, fg_p1, fg_p2, fg_p3, l_p1, l_p2, l_p3, l0_p2, l1_p2, l0_p3, l1_p3, l2_p3, n_g1, n_g2, n_g3, part_pd_score, matching_vector, wieighted_predict
