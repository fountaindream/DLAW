...
class Main():
    ...
    def evaluate(self):
        ...
        def rank(qf,ql,qc,qh,gf,gl,gc,gh):
            ...
            good_index = np.setdiff1d(query_index, camera_index, assume_unique=True) #返回不同元素
            junk_index1 = np.argwhere(gl==-1)
            junk_index2 = np.intersect1d(query_index, camera_index) # 返回相同元素
            #**************uncomment here for cloth-changing testing protocol********************
            # good_index = np.setdiff1d(good_index, cloth_index, assume_unique=True) #返回不同元素          
    ...
    if opt.mode == 'train':
        ...
        for epoch in range(1, opt.epoch + 1):
            ...
            if epoch % 50 == 0:
                print('\nstart evaluate')
                main.evaluate()
                os.makedirs('weights/LTCC/', exist_ok=True)
                torch.save(model.state_dict(), ('weights/LTCC/model_{}.pt'.format(epoch)))
